    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6 px-4 sm:px-6 lg:px-0">
                <h3 class="font-semibold text-lg text-gray-800">Manajemen Tagihan Pasien</h3>
            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 rounded-lg shadow-md">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">{{ $pasien->no_visit }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">{{ \Carbon\Carbon::parse($pasien->visit_date)->translatedFormat('d F Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">{{ $pasien->pasien->no_rekam_medis }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">{{ $pasien->pasien->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center {{ $pasien->visit_type == 'bpjs' ? 'uppercase' : 'capitalize' }}">
                                        <span class="px-2 py-1.5 rounded-md {{ $pasien->visit_type == 'bpjs' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600' }}">{{ $pasien->visit_type }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center space-x-2">
                                        <a href="{{ route('kasir.bayar.create', $pasien) }}" class="inline-block px-3 py-1.5 bg-blue-500 text-white text-xs font-semibold rounded-md hover:bg-blue-600">
                                            <i class="fa-solid fa-dollar-sign"></i> Bayar Tagihan
                                        </a>
                                ...
                                @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-gray-500">Data pasien tidak ditemukan.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
